...
#endif
#if defined(CONFIG_BUGi386) || defined(__arm__)
	{ "reboot=", reboot_setup },
#endif
#ifdef CONFIG_INET
	{ "ether=", eth_setup },
 ...
 */
...
#ifdef CONFIG_SYSCTL
	sysctl_init();
#if defined(__arm__) && defined(CONFIG_ALIGNMENT_TRAP)
	{ /* I really think a post_arch_init() would be a good idea,
	   * or else things are going to get dirty in the ARM world.
	   * This would replace most of the following #ifdef's
	   */
		extern void alignment_init(void);
		alignment_init();
	}
#endif
#endif
	...
	kernel_thread(khttpd, NULL, CLONE_FS | CLONE_FILES | CLONE_SIGHAND);
