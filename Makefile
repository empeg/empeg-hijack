...
PATCHLEVEL = 2
SUBLEVEL = 17
EXTRAVERSION = -rmk5-np17-empeg52-hijack-v418
...
$(patsubst %, _dir_%, $(SUBDIRS)) : dummy include/config/MARKER $(CONSTANTS)
...
modules_install:
	...
	if [ -f ACORN_NET_MODULES  ]; then inst_mod ACORN_NET_MODULES net;   fi; \
	if [ -f IPV4_MODULES       ]; then inst_mod IPV4_MODULES  ipv4;      fi; \
	if [ -f IPV6_MODULES       ]; then inst_mod IPV6_MODULES  ipv6;      fi; \
	if [ -f SCSI_MODULES       ]; then inst_mod SCSI_MODULES  scsi;      fi; \
	if [ -f ACORN_SCSI_MODULES ]; then inst_mod ACORN_SCSI_MODULES scsi; fi; \
...
dep-files: scripts/mkdep symlinks archdep include/linux/version.h new-genksyms
