	...
	unsigned long prot, unsigned long flags, unsigned long off)
{
	...
	/*
	printk("do_mmap: %s @ %08x len %08x prot %c%c%c flags %c%c offset %08x\n",
	       file ? "file" : "anon",
	       (int) addr,
	       (int) len,
	       prot & PROT_READ ? 'r' : ' ',
	       prot & PROT_WRITE ? 'w' : ' ',
	       prot & PROT_EXEC ? 'x' : ' ',
	       ((flags & VM_LOCKED) || (mm->def_flags & VM_LOCKED)) ? 'l' : ' ',
	...
	*/
	...
	if (!vma) {
		printk("can't kmem_cache_alloc (ENOMEM)\n");
		return -ENOMEM;
	}
	...
	if (do_munmap(addr, len)) {
		printk("couldn't munmap previous mappings\n");
		goto free_vma;
	}
	...
	if ((vma->vm_flags & (VM_SHARED | VM_WRITE)) == VM_WRITE &&
		...
		printk("no room for private writable mapping\n");
		goto free_vma;
	}
	...
	if (flags & VM_LOCKED) {
		...
		//		printk("success (paged, locked)\n");
#ifdef CONFIG_SA1100_EMPEG
	    }
	    //	    else
	    //	        printk("success (no read/write, unpaged, locked)\n");
#endif
	}
	//	else printk("success (unpaged, unlocked)\n");
	return addr;
