/*
 * linux/drivers/video/sa1100fb.c -- Strong Arm 1100 LCD Controller Frame Buffer Device
 *
...
int get_pcd(unsigned int pixclock)
{
	unsigned int pcd;
 ...
 */
void sa1100fb_inter_handler(int irq, void *dev_id, struct pt_regs *regs)
{
        if (LCSR & LCSR_LDD) {
...
sa1100fb_init(void))
{
	...
	DPRINTK("sa1100fb: request_irq succedded\n");
	disable_irq(IRQ_LCD);
...
__initfunc(void
sa1100fb_setup(char *options, int *ints))
