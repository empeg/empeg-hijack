...
static void ide_do_request (ide_hwgroup_t *hwgroup, int masked_irq)
{
	...
	while (!hwgroup->busy) {
		...
		if (hwgroup->hwif->sharing_irq && hwif != hwgroup->hwif)	/* set nIEN for previous hwif */
			OUT_BYTE(hwgroup->drive->ctl|2, hwgroup->hwif->hw.io_ports[IDE_CONTROL_OFFSET]);
		hwgroup->hwif = hwif;
		hwgroup->drive = drive;
		drive->sleep = 0;
 ...
 * do it for you.
 */
void ide_setup_ports(hw_regs_t *hw, ide_ioreg_t base, ide_ioreg_t *offsets,
		     ide_ioreg_t ctrl, ide_ioreg_t intr, ide_ack_intr_t *ack_intr,
		     int irq)
{
	int i;

	for (i = 0; i < IDE_NR_PORTS; i++) {
		if (offsets[i] == (ide_ioreg_t)-1) {
			switch(i) {
...
#if defined(CONFIG_SA1100_EMPEG) && !defined(CONFIG_EMPEG_HENRY)
// This incredibly dodgy hack was inserted by Hugo Fiennes to make the
// ide code identify two drives successfully.
		/* Hack! Hack! */
		/* Altered by MAC 23/4/1999 to swap the order of the drives */
  /* IDE2 is hda */
  if (hw->io_ports[IDE_DATA_OFFSET]!=0) hwif=&ide_hwifs[0]; else hwif=&ide_hwifs[1];
  goto found;
#endif
		for (index = 0; index < MAX_HWIFS; ++index) {
			hwif = &ide_hwifs[index];
...
int ide_register (int arg1, int arg2, int irq)
{
	...
	ide_init_hwif_ports(&hw, arg1, arg2, irq);
...
__initfunc(void ide_setup (char *s))
{
	...
	if (s[3] >= '0' && s[3] <= max_hwif) {
		...
		switch (i) {
			...
			case 3: /* base,ctl,irq */
				ide_init_hwif_ports(&hwif->hw, vals[0], vals[1], vals[2]);
				hwif->noprobe  = 0;
...
#else
	printk(KERN_ERR "%s -- NOT SUPPORTED ON ide%d\n", s, hw);
#endif
done:
#ifndef DEBUG
	printk("\n");
...
__initfunc(void ide_init_builtin_drivers (void))
{
	...
	if (ide_hwifs[0].hw.io_ports[IDE_DATA_OFFSET]) {
		...
		disable_irq(ide_hwifs[0].hw.irq);
	}
