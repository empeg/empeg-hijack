...
/* SA1100 ThinClient kludges */

#define CPLD_ST_IRQ  (*((volatile unsigned char *)0xDC080004))
#define CPLD_EN_IRQ  (*((volatile unsigned char *)0xDC08000C))
#define CPLD_IRQ  0x02
#define GP_IRQ  (1<<0)
...
static void smc_hardware_send_packet( struct device * dev )
{
	...
	*/
	...
	if ( (length & 1) == 0 ) {
		...
		SMC_outw( ((word)buf[length-1]) | 0x2000, DATA_1 );
	}
...
int __init smc_init(struct device *dev)
{
	...
	MSC1 |= 0x0000e000;  /* max recovery timing... */
	base_addr = 0xDC0E0000;

	error = smc_probe(base_addr);
	if ( !error )
		error = smc_initcard( dev, base_addr );
...
/* note: the old module interface does not support this call */
	...
	*/
	...
	GRER &= ~GP_IRQ;
      	...
      	irqval = request_irq(dev->irq, &smc_interrupt, SA_SHIRQ, CARDNAME, dev);
      	if (irqval) {
...
*/
		SMC_outw( SMC_inw( RCR ) & ~RCR_PROMISC, RCR );
		SMC_outw( SMC_inw( RCR ) | RCR_ALMUL, ioadddr, RCR );
	}
#else
