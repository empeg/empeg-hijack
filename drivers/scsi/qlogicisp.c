...
int isp1020_queuecommand(Scsi_Cmnd *Cmnd, void (*done)(Scsi_Cmnd *))
{
	...
	if (Cmnd->use_sg) {
		...
		for (i = 0; i < n; i++) {
			...
			dma_cache_wback_inv(sg->address, sg->length);
			++sg;
		}
		sg_count -= 4;
		...
		dma_cache_wback_inv(Cmnd->request_buffer, Cmnd->request_bufflen);
	}

	dma_cache_wback_inv(cmd, sizeof(struct Command_Entry));
...
static int isp1020_load_parameters(struct Scsi_Host *host)
{
	...
	dma_cache_wback_inv(&hostdata->res[0][0], RES_QUEUE_LEN + 1);
	...
	dma_cache_wback_inv(&hostdata->res[0][0], QLOGICISP_REQ_QUEUE_LEN + 1);
