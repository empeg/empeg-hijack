...
{
      if (PAGE_SIZE > 8192)
              return kmalloc(sizeof(struct tty_struct), GFP_KERNEL);
      else
              return (struct tty_struct *)get_free_page(GFP_KERNEL);
}
	...
	size_t count)
{
	...
	if ( test_bit(TTY_NO_WRITE_SPLIT, &tty->flags) )
		...
		for (;;) {
			size_t size = MAX(PAGE_SIZE*2,16384);
			if (size > count)
				size = count;
...
long __init console_init(long kmem_start, long kmem_end)
{
	...
	kmem_start = serial_sa1100_console_init(kmem_start, kmem_end);
#endif
#ifdef CONFIG_SERIAL_21285_CONSOLE
	kmem_start = rs285_console_init(kmem_start, kmem_end);
#endif
#ifdef CONFIG_3215
