...
#endif /* !defined (io_p2v) */

#include <asm/arch/cmptblty.h>
#include <asm/arch/bitfield.h>
#include <asm/arch/ARMv4.h>
#include <asm/arch/cp15.h>
...
#define UTCR2_BdRtDiv(Div)      	/*  Baud Rate Divisor [16..65536]  */ \
                	(((Div) - 16)/16 BAnd FAlnMsk (UTCR2_BRD) << \
                	 FShft (UTCR2_BRD))
...
#define UTCR2_CeilBdRtDiv(Div)  	/*  Ceil. of BdRtDiv [16..65536]   */ \
                	(((Div) - 1)/16 BAnd FAlnMsk (UTCR2_BRD) << \
                	 FShft (UTCR2_BRD))
...
#define SDCR4_BdRtDiv(Div)      	/*  Baud Rate Divisor [16..65536]  */ \
                	(((Div) - 16)/16 BAnd FAlnMsk (SDCR4_BRD) << \
                	 FShft (SDCR4_BRD))
...
#define SDCR4_CeilBdRtDiv(Div)  	/*  Ceil. of BdRtDiv [16..65536]   */ \
                	(((Div) - 1)/16 BAnd FAlnMsk (SDCR4_BRD) << \
                	 FShft (SDCR4_BRD))
...
#define DDAR_DevAdd(Add)        	/*  Device Address                 */ \
                	((Add) BAnd 0xF0000000 BOr \
                	 ((Add) BAnd 0X003FFFFC) << (FShft (DDAR_DA) - 2))
