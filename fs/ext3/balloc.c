 ...
 */
static int find_next_usable_block(int start, struct buffer_head *bh, int maxblocks)
{
	int here, next;
	char *p, *r;
	...
	while (here < maxblocks) {
		...
		here = ext3_find_next_zero_bit
			((unsigned long *) bh->b_committed_data, 
			 maxblocks, next);
	}
	return -1;
}
...
search_back:
	 ...
	 */
	for (k = 0; k < 7 && j > 0 && ext3_test_allocatable (j - 1, bh); k++, j--);
...
got_block:
	 ...
	 * us to this point! */
	J_ASSERT (!ext3_set_bit (j, bh->b_data));

	if (bh->b_committed_data)
		J_ASSERT(!ext3_test_bit(j, bh->b_committed_data));
	bhtmp = bh;
	alloctmp = j;
	...
	/* @@@ DEBUG At this point, if the buffer is still dirty
	 * metadata against the previous transaction, we have a
	 * problem. */
	if (bh->b_transaction == handle->h_transaction->t_journal->j_committing_transaction && bh->b_jlist == BJ_Metadata) {
		printk(KERN_ERR "ext3: bad buffer reuse on bitmap %p, bit %d\n", bhtmp, alloctmp);
		printk(KERN_ERR "ext3: bh %p, status (data/committed/frozen) %d %d %d\n", 
		       bh,
		       ext3_test_bit(alloctmp, bhtmp->b_data),
		       bhtmp->b_committed_data ? ext3_test_bit(alloctmp, bhtmp->b_committed_data) : -2,
		       bhtmp->b_frozen_data ? ext3_test_bit(alloctmp, bhtmp->b_frozen_data) : -2);
		BUG();
	}
	
	memset(bh->b_data, 0, sb->s_blocksize);
	mark_buffer_uptodate(bh, 1);
	bh->b_alloc2_transaction = bh->b_alloc_transaction;
	bh->b_alloc_transaction = handle->h_transaction->t_tid;
