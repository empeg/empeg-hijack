				...
				size_t count, loff_t *ppos)
{
	...
	unsigned long limit;
	handle_t *handle;
	int will_journal_data, ordered_data;
	 ...
	 * ordered or writeback journaling (eg. the quota files).
	 *
	 * ordered_data is set if we are using ordered data for writes,
	 * OR if O_SYNC is specified.  In the latter case, we'll
	 * automatically trigger a data writeback as a result of the
	 * commit-on-journal_stop. */
	will_journal_data = ext3_should_journal_data(inode);
	ordered_data = ((test_opt3(inode->i_sb, DATA_FLAGS) == 
			 EXT3_MOUNT_ORDERED_DATA) || 
			(filp->f_flags & O_SYNC));		
	
	jfs_debug(6, "will_journal_data is %d, ordered_data is %d\n", 
		  will_journal_data, ordered_data);
	...
	do {
		...
		if (handle->h_buffer_credits < EXT3_RESERVE_TRANS_BLOCKS &&
		    written_transaction >= (EXT3_MAX_TRANS_DATA/2)) {
			needed = (count >> EXT3_BLOCK_SIZE_BITS(sb)) + 1;
			if (needed > EXT3_MAX_TRANS_DATA)
				needed = EXT3_MAX_TRANS_DATA;
