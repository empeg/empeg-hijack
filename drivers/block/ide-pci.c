...
check_if_enabled:
	...
	for (port = 0; port <= 1; ++port) {
		...
		if (hwif->hw.io_ports[IDE_DATA_OFFSET] != base) {
			ide_init_hwif_ports(&hwif->hw, base, ctl + 2, hwif->hw.irq);
...
__initfunc(void ide_scan_pcibus (void))
{
	...
	for(dev = pci_devices; dev; dev=dev->next) {
		...
		else if (IDE_PCI_DEVID_EQ(d->devid, DEVID_82C693) && PCI_FUNC(dev->devfn) != 1 &&
			 PCI_FUNC(dev->devfn) != 2)
			continue;	/* 82C693 uses same devid for functions 0 to 3 */
