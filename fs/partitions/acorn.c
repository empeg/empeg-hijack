		      ...
		      unsigned long first_sector, unsigned int minor)
{
	 ...
	 *
	 * There are unknowns in this code - is the 'cylinder number' of
	 * the next partition relative to the start of this one - I'm
	 * assuming it is.
	 *
	 * Also, which ID did Cumana use?
	 *
	 * This is totally unfinished, and will require more work to get
	 * it going.  Hence it is totally untested.
	 */
	do {
		struct disc_record *dr;
		unsigned int nr_sects;
 ...
 */
static int
adfspart_check_ADFS(struct gendisk *hd, kdev_t dev, unsigned long first_sector,
		    unsigned int minor)
{
		   ...
		   unsigned long first_sector, unsigned int minor)
{
	...
	if (sum != le32_to_cpu(*(__u32 *)(&bh->b_data[508]))) {
	    	brelse(bh);
	...
	for (p = (struct ics_part *)bh->b_data; p->size; p++) {
		...
		if (size < 0) {
			 ...
			 * this partition is...
			 */
			if (size > 1 && adfspart_check_ICSLinux(dev, p->start)) {
				start += 1;
				size -= 1;
			}
		}
		...
		if (size) {
			add_gd_partition(hd, minor, first_sector +
					 start, size, 0);
			minor++;
		}
	}
			...
			unsigned long first_sector, unsigned int minor)
{
	...
	for (i = 0, p = (struct ptec_partition *)bh->b_data;
	     i < 12; i++, p++) {
