...
#if defined(CONFIG_FB) || defined(CONFIG_VGA_CONSOLE)
...
void handle_scancode(unsigned char scancode, int down)
{
	...
	/*
	 *  Convert scancode to both keycode
	 */
...
static void kbd_bh(void)
{
	...
	extern int ignore_kbd_irq;

	if (rep) {
		tty = ttytab? ttytab[fg_console]: NULL;
		kbd = kbd_table + fg_console;
		ignore_kbd_irq = 1;
		if (kbd_repeatkeycode != -1)
...
#ifndef CONFIG_FOOTBRIDGE
__initfunc(char *kbd_reset_setup(char *s))
{
	return s;
}
#endif
