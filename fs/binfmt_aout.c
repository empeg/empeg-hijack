...
static inline int do_load_aout_binary(struct linux_binprm * bprm, struct pt_regs * regs)
{
	...
	if (N_MAGIC(ex) == OMAGIC) {
		...
		error = do_mmap(NULL, text_addr & PAGE_MASK, map_size,
				PROT_READ|PROT_WRITE|PROT_EXEC,
				MAP_FIXED|MAP_PRIVATE, 0);
		...
		error = read_exec(bprm->dentry, fd_offset, (char *)text_addr,
		...
		if (fd < 0) { /* RMK: can't return to calling process... send it a SIGKILL */
			send_sig(SIGKILL, current, 0);
			return fd;
		}
		file = fget(fd);
...
#else
	start_addr = ex.a_entry & 0xffff8000;	/* shouldn't this be in a header file? */
