...
#ifdef CONFIG_SA1100_EMPEG
	/* on empeg-car we need to lock down quite a lot */
	/* so we allow locking up to 7/8 of memory.      */
	if (current->mm->total_vm > num_physpages - num_physpages/8)
		goto out;
#else
...
asmlinkage int sys_mlockall(int flags)
{
	...
	/*
	if (current->mm->total_vm > num_physpages - num_physpages/8) {
		printk("Cannot lock %lu pages total\n", (unsigned long) current->mm->total_vm);
		goto out;
	}
	*/
#else
