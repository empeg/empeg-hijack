 ...
 */
static inline void free_one_pmd(pmd_t * dir, int dont_free_zero)
{
	pte_t * pte;
	...
	/* On ARM, only free first page of memory
	 * on a free_page_tables()
	 */
	if (dont_free_zero)
		return;
...
static inline void free_one_pgd(pgd_t * dir, int dont_free_zero)
{
	int j;
	pmd_t * pmd;
	...
	for (j = 0; j < PTRS_PER_PMD ; j++)
		free_one_pmd(pmd+j, dont_free_zero && j == 0);
...
void clear_page_tables(struct mm_struct *mm, unsigned long first, int nr)
{
	...
	if (page_dir && page_dir != swapper_pg_dir) {
		...
		if (!first) {
			nr -= 1;
			free_one_pgd(page_dir, 1);
			page_dir++;
		}
		if (nr)
#endif
		do {
			free_one_pgd(page_dir, 0);
			page_dir++;
		} while (--nr);
...
int new_page_tables(struct task_struct * tsk)
{
	...
	if (!pmd_alloc(new_pg, 0))
		return -ENOMEM;
	else {
		pmd_t *new_pmd = pmd_offset(new_pg, 0);
		pmd_t *old_pmd = pmd_offset(pgd_offset(&init_mm, 0), 0);
...
/* no need for flush_tlb */
	update_memc_addr(tsk->mm, address, *pte);
	return page;
}
	...
	unsigned long address, pte_t *page_table)
{
	...
	case 1:
		...
		update_memc_addr(vma->vm_mm, address, *page_table);
end_wp_page:
		/*
	...
	unsigned long address, int write_access, pte_t *page_table)
{
	...
	if (!vma->vm_ops || !vma->vm_ops->nopage) {
		int ret;
		unlock_kernel();
		ret = do_anonymous_page(tsk, vma, page_table, write_access,
					address);
		if (ret)
			update_memc_addr(vma->vm_mm, address, *page_table);
		return ret;
	...
	int write_access, pte_t * pte)
{
	...
	update_memc_addr(vma->vm_mm, address, entry);
	unlock_kernel();
	return 1;
}
	...
	unsigned long address, int write_access)
{
	...
	if (ret > 0) {
		...
		update_memc_addr(vma->vm_mm, address, *pte);
	}
	return ret;
}
