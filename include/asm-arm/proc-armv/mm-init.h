...
#else
	/* On SA1100 we need B and C set to hit the main cache */
	alloc_init_section(&start_mem, FLUSH_BASE, FLUSH_BASE_PHYS, DOMAIN_KERNEL,
			   PMD_SECT_BUFFERABLE | PMD_SECT_CACHEABLE | PMD_SECT_AP_READ);
	/* and C only for the minicache...  well...  */
	alloc_init_section(&start_mem, FLUSH_BASE_MINICACHE, FLUSH_BASE_PHYS,
