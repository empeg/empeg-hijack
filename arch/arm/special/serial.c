 ...
 *
 * It is also used at boot time for the ARM Acorn architectures to
 * allow them to register their expansion cards.
 */
int register_serial(struct serial_struct *req)
{
	int i;
...
#ifdef __arm__
	/* On some cards, we need to specify the baud rate
	 * we pass it in with the IO parameters (since it is
	 * part of the physical hardware configuration).
	 * Maybe all drivers ought to set baud_base to their
	 * explicit BAUD_BASE value, in which case we can
	 * then get rid of the #ifdef __arm__ - RMK
	 */
	if (req->baud_base)
