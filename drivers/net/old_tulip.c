...
static const char version[] = "tulip.c:v0.90 10/20/98 becker@cesdis.gsfc.nasa.gov+v0.190 981105 FASTROUTE/HW_FLOWCONTROL\n";
...
/* Some possibilities for the interested user -- mutually exclusive */
#undef FILL_RX_RING_AT_ONCE 
#undef HW_FLOWCONTROL_NEW
...
#define dma_wback_t(obj) \
	dma_cache_wback((unsigned long)(obj), sizeof(*(obj)))
#define dma_wback_inv_t(obj) \
	dma_cache_wback_inv((unsigned long)(obj), sizeof(*(obj)))
#define dma_inv_t(obj) \
	dma_cache_inv((unsigned long)(obj), sizeof(*(obj)))
...
#ifdef AVE_STATS
#define AVE_STATS_VALUES 10 /* 5*2 */

static void ave_get(unsigned long arg);

struct ave_estimate
{
	unsigned int rx_packets_5min;
	unsigned int tx_packets_5min;
	unsigned int rx_bytes_5min;
	unsigned int tx_bytes_5min;
	unsigned int rx_errors_5min;
	unsigned int tx_errors_5min;
	unsigned int rx_dropped_5min;
	unsigned int tx_dropped_5min;
	unsigned int collisions_5min;
	unsigned int rx_missed_errors_5min;
	unsigned long time;
};
#endif
...
#ifdef EXTRA_STATS
	unsigned long stats_tx_new_refilled; /* # Packets filled with new scheme */
	unsigned long stats_tx_old_refilled; /* # Packets filled with old scheme */

#ifdef CONFIG_NET_HW_FLOWCONTROL
	unsigned long stats_fc_on; /* HW_FLOWCONTROL # of xon's */
#endif

	unsigned long stats_private_skb_hits; 
	unsigned long stats_private_skb_misses; 
...
#ifdef PPS_TESTING
	/* Special for measurement */
	unsigned long stats_pktgun_pkts; /* Received pkts frpm packet-gun */
#endif

#endif /* EXTRA_STATS */

#ifdef CONFIG_NET_FASTROUTE
	unsigned long stats_fastroute_hits;
	unsigned long stats_fastroute_succeed;
	unsigned long stats_fastroute_deferred;
#endif		
...
#ifdef AVE_STATS
	struct ave_estimate ave_buf[AVE_STATS_VALUES];
	struct timer_list ave_timer;
	unsigned int ring_p;
	unsigned int rx_packets_5min;
	unsigned int tx_packets_5min;
	unsigned int rx_bytes_5min;
...
int  c_time(char *buf, unsigned i); 
int dev_proc_tulip(char *buf , char **start, off_t offset,
                          int len, int unused);
#include <linux/proc_fs.h>

static struct proc_dir_entry tulip_proc_entry = {
        PROC_NET_TULIP, 5, "tulip", 
        S_IFREG | S_IRUGO, 1, 0, 
        0, 0, 
		NULL, 
		dev_proc_tulip
...
__inline__ static int tx_ring_fill(struct device *dev);
__inline__ static int rx_ring_fill(struct device *dev);
...
#ifdef CONFIG_NET_FASTROUTE
#undef FAST_SKB_RECYCLE /* Don't define it-- it's not working yet */
...
#define TULIP_RECYCLE_LIST_SIZE 100   /* Max # private buffers */

unsigned long recycle_hits, recycle_misses;
__u32 dev_alloc_stats[4];
__u32 frames_per_rx[4]; /* Min, Max, #frames, #rx */
__u32 tx_stats[6]; 

struct sk_buff_head tulip_recycle_list;

static __inline__ void tulip_recycle_skb(struct sk_buff *skb)
{
	if (skb->pkt_type == PACKET_FASTROUTE &&
		skb->type == 1 && 
		tulip_recycle_list.qlen <= TULIP_RECYCLE_LIST_SIZE) {
		skb->type = 0;
		skb->tail = skb->data;
		skb->len = 0;
		/* LIFO queue */
		skb_queue_head(&tulip_recycle_list, skb);
	} else
		kfree_skb(skb);
}

static __inline__  struct sk_buff * get_recycle_skb(unsigned int size)
{
	struct sk_buff *skb;	
	skb =  skb_dequeue(&tulip_recycle_list );
	if (skb) recycle_hits++;
	else recycle_misses++;
	return skb;
}
...
int tulip_probe(struct device *dev)
{
	...
	if( cards_found ) {
#if 0 //def CONFIG_PROC_FS
		proc_net_register(&tulip_proc_entry);
#endif
#ifdef FAST_SKB_RECYCLE
       if (tulip_recycle_list.next == NULL)
                       skb_queue_head_init(&tulip_recycle_list);
#endif
	}
...
#else
		outl(0x08210008, ioaddr+CSR15);
		udelay(1);
		outl(0x00010008, ioaddr+CSR15);
		udelay(2);
...
static struct fixups {
  ...
  {"Corel", 0, 0x10, 0x57, { 0x1e00, 0x0000,

				0x0800,	/* default media (autosense) */
				0x8f01, /* 1 media types, first 11 bytes */
				   0x0103, 0x0000, 0x0000, 0x01e1
  }},
  {0, 0, 0, 0, {}}};
...
media_picked:
	...
	/*
	  printk( "Setting interrrupt mask 0x%x\n", 
	...
	*/
	...
	{
		int p1;
		for(p1=0; p1< AVE_STATS_VALUES; p1++) 
			tp->ave_buf[p1].time= jiffies;
		
		tp->ring_p = 0;
		init_timer(&tp->ave_timer);
		tp->ave_timer.expires = jiffies + 1*HZ;
		tp->ave_timer.function = ave_get;
		tp->ave_timer.data = (unsigned long) tp;
		add_timer(&tp->ave_timer);
	}
#endif	

#ifdef EXTRA_STATS
	dev->trans_start = dev->last_rx = tp->open_time = jiffies;
#endif
...
static void t21142_timer(unsigned long data)
{
	...
	if (media_cap[dev->if_port] & MediaIsMII) {
		...
		if (!(csr12 & 4)) {		/* 10mbps link beat good. */
			...
			outl(t21142_csr15[dev->if_port], ioaddr + CSR15);
#else
			outl(t21142_csr15[dev->if_port] | 0x10000, ioaddr + CSR15);
#endif
			outl(t21142_csr13[dev->if_port], ioaddr + CSR13);
		} else if (csr12 & 0x100) {
			new_csr6 = 0x82420200;
...
tulip_start_xmit(struct sk_buff *skb, struct device *dev)
{
	...
	/* cli()/sti() moved to lock even when FASTROUTE is not defined 
	   980913 --ro */
	...
	/* See if we can free slots on the output ring */
	/*	if(tp->cur_tx - tp->dirty_tx == TX_RING_SIZE/2 ) */

#ifdef EXTRA_STATS
	tp->stats_tx_new_refilled += 
#endif		
	  tx_ring_fill(dev);
	...
	/*RMK*/
	dma_wback_t(&tp->tx_ring[entry]);
	tp->tx_ring[entry].status = 0x80000000;	/* Pass ownership to the chip. */
	tp->cur_tx++;

	/*RMK*/
	dma_wback_t(&tp->tx_ring[entry].status);
	...
	do {
...
#ifdef EXTRA_STATS
		if( csr5 & NormalIntr ) tp->stats_intr_normal++;
		if( csr5 & AbnormalIntr ) tp->stats_intr_abnormal++;
		if( csr5 & RxIntr ) tp->stats_intr_rx++;
		if( csr5 & RxNoBuf ) tp->stats_intr_rx_nobuf++;
		if( csr5 & TxIntr ) tp->stats_intr_tx++;
		if( csr5 & TxDied ) tp->stats_intr_tx_died++;
		if( csr5 & TxNoBuf ) tp->stats_intr_tx_nobuf++;
#endif
		...
		if (csr5 & (TxNoBuf | TxDied | TxIntr)) {
			...
			for (dirty_tx = tp->dirty_tx; tp->cur_tx - dirty_tx > 0;
				...
				/*RMK*/
				dma_wback_inv_t(&tp->tx_ring[entry].status);

				status = tp->tx_ring[entry].status;
...
#ifdef FAST_SKB_RECYCLE
				tulip_recycle_skb(xchg(&tp->tx_skbuff[entry], NULL)); 
#else
		...
		if (csr5 & AbnormalIntr) {	/* Abnormal error summary bit. */
			...
			if (csr5 & TxFIFOUnderflow) {
				...
				/* Restart the transmit process. */
				if (tulip_debug) 
					printk(KERN_WARNING "%s: The transmitter stopped pos2 !"
						   "  CSR5 is %x, CSR6 %x.\n",
						   dev->name, csr5, inl(ioaddr + CSR6));
				outl(tp->csr6 | 0x0002, ioaddr + CSR6);
				outl(tp->csr6 | 0x2002, ioaddr + CSR6);
...
#ifdef FILL_RX_RING_AT_ONCE
			if (1) {
#ifdef FAST_SKB_RECYCLE
				struct sk_buff *skb2 = tp->rx_skbuff[entry] = 
					get_recycle_skb(PKT_BUF_SZ);
				if( !skb2 ) skb2 = dev_alloc_skb(PKT_BUF_SZ);
#else
				struct sk_buff *skb2 = dev_alloc_skb(PKT_BUF_SZ);
#endif
				if (skb2 == NULL) return 3; /* oom */
				tp->rx_ring[entry].buffer1 = virt_to_bus(skb2->tail);
				skb2->dev = dev;
				tp->rx_skbuff[entry] = skb2;
#ifdef FAST_SKB_RECYCLE
				skb2->type = 1;
#endif
			}
#else
			tp->rx_skbuff[entry] = NULL;  /* Fill it later */
#endif
			skb_put(skb, len);
...
tulip_rx(struct device *dev)
{
	...
	while (tp->rx_ring[entry].status >= 0) {
		...
		if ((status & 0x38008300) != 0x0300) {
			...
			case 1:
				goto semi_gone;
			case 2:     /* fast path is hit, but packet is a bit strange */
				break;
			case 3:     /* oom */
				break;
			case 4:     /* fast path miss. */
				break;
			}
#endif
			...
			if ( pkt_len < rx_copybreak 
				...
				unsigned long virt_buf_addr = bus_to_virt(tp->rx_ring[entry].buffer1);;

				skb->dev = dev;
#if ! defined(__alpha__)
				dma_cache_inv(virt_buf_addr, pkt_len);
				skb_reserve(skb, 2);	/* 16 byte align the IP header */
				eth_copy_and_sum(skb, virt_buf_addr, pkt_len, 0);
				skb_put(skb, pkt_len);
#else
 				memcpy(skb_put(skb, pkt_len), virt_buf_addr, pkt_len);
#endif
				work_done++;
      ...
      throttle:
          if (tp->fc_bit) {
#ifndef HW_FLOWCONTROL_NEW
			  outl(tp->csr6 | 0x2000, dev->base_addr + CSR6); 
#endif
	...
	for (; tp->cur_rx - tp->dirty_rx > 0; tp->dirty_rx++) {
		...
		if (tp->rx_skbuff[entry] == NULL) {
			struct sk_buff *skb;
#ifdef FAST_SKB_RECYCLE
			skb = tp->rx_skbuff[entry] = get_recycle_skb(PKT_BUF_SZ);
			if( !skb )
#endif
		...
		dma_wback_t(&tp->rx_ring[entry]);
		tp->rx_ring[entry].status = 0x80000000;
		dma_wback_t(&tp->rx_ring[entry].status);
	}
#ifdef HW_FLOWCONTROL_NEW
	 }
#endif
	return work_done;
oom: 
	/* take care of when we are out memory */
	/* I have now idea for now */
	if( net_ratelimit()) printk( KERN_WARNING "oom\n");
	return work_done;
}
__inline__ static int rx_ring_fill(struct device *dev)
{
...
__inline__ static int tx_ring_fill(struct device *dev)
{
    ...
    /* Can we free any TX-buffers now? 
    */
	...
	for (dirty_tx = tp->dirty_tx; tp->cur_tx-dirty_tx>0; dirty_tx++) {
		...
		free++;
		/*
		if( net_ratelimit()) 	printk( "In tx_ring_fill free %d\n", free);
		*/
    }
    ...
    if (tp->tx_full && dev->tbusy
		...
		printk("The ring is no longer full, clear tbusy\n");
...
tulip_close(struct device *dev)
{
	...
	for (i = 0; i < RX_RING_SIZE; i++) {
		...
		dma_wback_t(&tp->rx_ring[i].status);
		tp->rx_ring[i].length = 0;
		tp->rx_ring[i].buffer1 = 0xBADF00D0; /* An invalid address. */
		dma_wback_t(&tp->rx_ring[i]);
		if (skb) {
#if LINUX_VERSION_CODE < 0x20100
			skb->free = 1;
#endif
	...
	if (dev->flags & IFF_PROMISC) {			/* Set promiscuous. */
		...
		if (tp->cur_tx - tp->dirty_tx > TX_RING_SIZE - 2) {
			...
			if (dummy > 0) {
				...
				dma_wback_t(&tp->tx_ring[dummy].status);
			}
			restore_flags(flags);
			/* Trigger an immediate transmit demand. */
			outl(0, ioaddr + CSR1);
...
static void ave_get(unsigned long arg)
{
	struct tulip_private *tp = (struct tulip_private *) arg;
	unsigned long ent;
	ent = tp->ring_p % AVE_STATS_VALUES;
	tp->ave_buf[ent].time = jiffies;

	tp->ave_buf[ent].rx_packets_5min = tp->stats.rx_packets;
	tp->ave_buf[ent].tx_packets_5min = tp->stats.tx_packets;
	tp->ave_buf[ent].rx_bytes_5min = tp->stats.rx_bytes;
	tp->ave_buf[ent].tx_bytes_5min = tp->stats.tx_bytes;
	tp->ave_buf[ent].rx_errors_5min = tp->stats.rx_errors;
	tp->ave_buf[ent].tx_errors_5min = tp->stats.tx_errors;
	tp->ave_buf[ent].rx_dropped_5min = tp->stats.rx_dropped;
	tp->ave_buf[ent].tx_dropped_5min = tp->stats.tx_dropped;
	tp->ave_buf[ent].collisions_5min = tp->collisions_5min;
	/* Fix this... has to read missed counter on chip */
	tp->ave_buf[ent].rx_missed_errors_5min = tp->stats.rx_missed_errors;

	tp->ring_p++;

#ifdef NOTNOW
	init_timer(&tp->ave_timer);
	tp->ave_timer.expires = jiffies + HZ*30;
	tp->ave_timer.function = ave_get;
	tp->ave_timer.data = (unsigned long) &tp;
#endif
	tp->ave_timer.expires = jiffies + HZ*30;
...
static void  ave_calc(struct device *dev)
{
	struct tulip_private *tp = (struct tulip_private *)dev->priv;
	unsigned long diff;
	int ent;
	...
	ent = (tp->ring_p) % AVE_STATS_VALUES;
	if( ent < 0 ) ent = 0;
	diff = (jiffies - tp->ave_buf[ent].time)/HZ;
	if( diff == 0 ) diff=1;

	tp->rx_packets_5min = 
		(tp->stats.rx_packets-tp->ave_buf[ent].rx_packets_5min)/diff;
	tp->tx_packets_5min = 
		(tp->stats.tx_packets-tp->ave_buf[ent].tx_packets_5min)/diff;
	tp->rx_bytes_5min = 
		(tp->stats.rx_bytes-tp->ave_buf[ent].rx_bytes_5min)/diff;
	tp->tx_bytes_5min = 
		(tp->stats.tx_bytes-tp->ave_buf[ent].tx_bytes_5min)/diff;
	tp->rx_errors_5min = 
		(tp->stats.rx_errors-tp->ave_buf[ent].rx_errors_5min)/diff;
	tp->rx_errors_5min = 
		(tp->stats.rx_errors-tp->ave_buf[ent].rx_errors_5min)/diff;
	tp->rx_dropped_5min = 
		(tp->stats.rx_dropped-tp->ave_buf[ent].rx_dropped_5min)/diff;
	tp->tx_dropped_5min = 
		(tp->stats.tx_dropped-tp->ave_buf[ent].tx_dropped_5min)/diff;
	tp->collisions_5min = 
		(tp->stats.collisions-tp->ave_buf[ent].collisions_5min)/diff;
	tp->rx_missed_errors_5min = 
		(tp->stats.rx_missed_errors-tp->ave_buf[ent].rx_missed_errors_5min)/diff;
}
#endif
                          ...
                          int len, int unused)
{
	...
	for (devp = &root_tulip_dev; *devp; devp = next) {
		...
		p += sprintf(p, "%s: %s %s %s ", dev->name, 
					 dev->flags&IFF_UP ? "UP" : "DOWN",
					 medianame[dev->if_port], 
					 tp->full_duplex ? "Full Duplex" : "Half Duplex" );
		{
			int new_reg1 = mdio_read(dev, tp->phys[0], 1);
			p += sprintf(p, "%s", new_reg1 & 0x0004 ? 
						 "Link UP\n" : "*** Link DOWN ***\n");
		}
...
#ifdef EXTRA_STATS
		if( dev->flags&IFF_UP ) {
			p += sprintf(p, "Admin up    " );
			p += c_time(p, (jiffies-(tp->open_time))/HZ);
			p += sprintf(p, "\n");
		}			
#endif
		p += sprintf(p, "Last input  " );
		p += c_time(p, (jiffies-(dev->last_rx))/HZ);
		p += sprintf(p, "\n");

		p += sprintf(p, "Last output " );
		p += c_time(p, (jiffies-(dev->trans_start))/HZ);
		p += sprintf(p, "\n");
		...
		ave_calc(dev);
		p += sprintf(p, "5min RX bits/s\t%-12u \n", 
					 tp->rx_bytes_5min*8);
		p += sprintf(p, "5min TX bits/s\t%-12u \n", 
					 tp->tx_bytes_5min*8);
		p += sprintf(p, "5min RX pkts/s\t%-12u \n", 
					 tp->rx_packets_5min);
		p += sprintf(p, "5min TX pkts/s\t%-12u \n", 
					 tp->tx_packets_5min);

		p += sprintf(p, "5min TX errors\t%-12u \n", 
					 tp->tx_errors_5min);
		p += sprintf(p, "5min RX errors\t%-12u \n", 
					 tp->rx_errors_5min);
		p += sprintf(p, "5min RX dropped\t%-12u \n", 
					 tp->rx_dropped_5min);
		p += sprintf(p, "5min TX dropped\t%-12u \n", 
					 tp->rx_dropped_5min);

		p += sprintf(p, "5min collisions\t%-12u \n", 
					 tp->collisions_5min);
		p += sprintf(p, "5min RX missed errors\t%-12u \n", 
					 tp->rx_missed_errors_5min);
#endif
		p += sprintf(p, "NormalIntr\t %-12lu\n",  tp->stats_intr_normal);
		p += sprintf(p, "AbnormalIntr\t %-12lu\n",  tp->stats_intr_abnormal);
		p += sprintf(p, "RxIntr\t %-12lu\n",  tp->stats_intr_rx);
		p += sprintf(p, "RxNoBuf\t %-12lu\n",  tp->stats_intr_rx_nobuf);
		p += sprintf(p, "TxIntr\t %-12lu\n",  tp->stats_intr_tx);
		p += sprintf(p, "TxDied\t %-12lu\n",  tp->stats_intr_tx_died);
		p += sprintf(p, "TxNoBuf\t %-12lu\n",  tp->stats_intr_tx_nobuf);

		p += sprintf(p, "TX-refilled with old/new scheme %lu/%lu \n", 
					 tp->stats_tx_old_refilled,
					 tp->stats_tx_new_refilled);
#ifdef CONFIG_NET_HW_FLOWCONTROL
#ifndef HW_FLOWCONTROL_NEW
		p += sprintf(p, "HW_flowcontrol(old)  on's\t %-12lu\n", 
					 tp->stats_fc_on);
#else
		p += sprintf(p, "HW_flowcontrol(new)  on's\t %-12lu\n", 
					 tp->stats_fc_on);
#endif
#endif
#ifdef PPS_TESTING	   
		p += sprintf(p, "RX-Pkts from pg\t %-12lu\n",  tp->stats_pktgun_pkts);
#endif /*PPS_TESTING*/
#endif

#ifdef CONFIG_NET_FASTROUTE
	p += sprintf(p, "fastroute pkts hits/succeed/deferred %lu/%lu/%lu\n",
				 tp->stats_fastroute_hits, tp->stats_fastroute_succeed, 
				 tp->stats_fastroute_deferred);
#endif		
	   p += sprintf(p, "\n");
	}
#ifdef FAST_SKB_RECYCLE
	p += sprintf(p, "skb statistics recycled/allocated  %lu/%lu\n",
				 recycle_hits, recycle_misses);
#endif
	return p-buf;
}
...
int  c_time(char *buf, unsigned i)
{
	...
	if( d ) p += sprintf(p, "%d day(s) ", d );
	if( h ) p += sprintf(p, "%d hour(s) ", h );
	if( m ) p += sprintf(p, "%d min ", m );
	if( s ) p += sprintf(p, "%d sec ", s );
	if( !d && !h && !m && !s )  p += sprintf(p, "NOW");
	return p-buf;
}
...
#ifdef FAST_SKB_RECYCLE
	printk( KERN_INFO "Freeing tulip_recycle_list size was %lu\n", 
			(long unsigned int)skb_queue_len(&tulip_recycle_list));
	skb_queue_purge(&tulip_recycle_list);
#endif
#if 0 //def CONFIG_PROC_FS
	proc_net_unregister(tulip_proc_entry.low_ino);
#endif
}
